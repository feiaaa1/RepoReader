...
import { MessageCircle, Copy, Check, Square, RotateCcw } from "lucide-react";
...
请稍候...`,
	...
	const { apiKey, selectedModel, userProfile, knowledgeBase, autoAnalysis } =
		useSettingsStore();

	// 当仓库数据加载完成时，根据设置决定是否自动发送项目分析请求
	useEffect(() => {
		if (
			repoData &&
			!isInitializing &&
...
2. **内容生成 (Generation)**:
   ...
   - **第二部分：🚀 项目宏观解读**
       ...
       - 此部分应包含以下三个小节：
		   ...
		   - **项目运行流程 (Workflow)**：
		       - 描述项目是如何启动的，站在使用者的角度阐述这个项目的运行过程，比如对于一个埋点系统，它的运行流程就是，npm下载 -> 在代码中初始化 -> 网站运行时上报埋点数据 -> 上报到服务端 -> 服务端分析处理。
...
💡 **点击右侧的"分析"按钮开始生成个性化的前置知识讲解**，或者直接向我提问关于这个项目的任何问题。`,
						role: "assistant",
						timestamp: new Date(),
					},
				]);
					...
					prev.map((msg) => {
						if (msg.id === analysisMessageId) {
							...
							if (msg.content.trim()) {
								...
								return {
									...msg,
									content: `⏹️ **已停止**: 分析已被用户中止。\n\n你可以重新开始分析或直接向我提问关于这个项目的任何问题。`,
								};
							}
						}
						return msg;
...
请现在开始分析我提供的信息，并生成报告。`;
		...
		// 发送前置知识讲解请求
		await handleAutoAnalysis(knowledgePrompt);
	};
	...
	const handleRegenerateMessage = async (messageId: string) => {
		...
		// 如果是分析消息，使用分析提示词
		if (messageId.startsWith("analysis-")) {
			if (!repoData) return;
...
请现在开始分析我提供的信息，并生成报告。`;
					...
					prev.map((msg) => {
						if (msg.id === assistantMessageId) {
							...
							if (msg.content.trim()) {
								...
								// 如果没有内容，显示停止消息
								return { ...msg, content: `⏹️ **已停止**: 对话已被用户中止。` };
							}
						}
						return msg;
					})
	...
	return (
		<div className="flex flex-col h-full">
			{/* 消息列表 */}
			<div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-10 scrollbar-thin">
								...
								{message.role === "assistant" ? (
									<div
										className={cn(
											...
											// 移除默认的 code 和 pre 样式，因为我们使用自定义的 CodeBlock 组件
											"prose-code:not([class*='language-']):text-blue-600 prose-code:not([class*='language-']):bg-blue-50 prose-code:not([class*='language-']):px-1 prose-code:not([class*='language-']):rounded prose-code:not([class*='language-']):text-sm",
											"prose-pre:hidden" // 隐藏默认的 pre 标签，使用自定义的 CodeBlock
										)}
									>
										<Markdown
											components={{
								...
								<div className="absolute -bottom-6 left-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
									<div className="flex gap-2">
										<button
											...
											className={cn(
												"flex items-center gap-1 px-1.5 py-0.5 text-xs rounded-md transition-colors",
