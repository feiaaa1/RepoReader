...
// 获取仓库README内容
export const getRepoReadme = async (owner: string, repo: string, branch: string = "main"): Promise<string> => {
    try {
...
// 获取仓库文件结构
export const getRepoStructure = async (owner: string, repo: string, branch: string = "main"): Promise<any[]> => {
    try {
...
export const generateRepoAnalysis = (repoData: RepoData): string => {
    ...
    const hasRequirementsTxt = files.some((f: any) => f.path === "requirements.txt");
    ...
    const totalDirs = structure.filter((item: any) => item.type === "tree").length;
