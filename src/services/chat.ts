...
// 生成系统提示词
function generateSystemPrompt(repoData?: RepoData, userProfile?: string, knowledgeBase?: string): string {
...
- 保持专业和友好的语调`;
    ...
    if (userProfile) {
        const profileMap: Record<string, string> = {
            'beginner': '用户是技术初学者，请用简单易懂的语言解释，避免过于复杂的技术术语',
...
项目结构概览：
${structure.slice(0, 20).map((file: any) => `- ${file.path}`).join('\n')}
